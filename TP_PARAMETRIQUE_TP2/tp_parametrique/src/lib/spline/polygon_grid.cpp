...
void polygon_grid::set_cylinder(int const size_u_param,int const size_v_param, float h_param, float r_param){
    resize(size_u_param+3,size_v_param);
    ...
    for(int ku=0;ku<=size_u_param+2;++ku){
...
void polygon_grid::set_boule(int const size_u_param,int const size_v_param, float r_param){
    resize(size_u_param+3,size_v_param+3);

    float pas_theta_u = 2*M_PI/size_u_param;
    float pas_theta_v = M_PI/(size_v_param);

    auto& S=*this;

    for(int kv=0;kv<size_v_param+3;++kv){
        float z = r_param*sin(kv*pas_theta_v-M_PI/2-pas_theta_v);
        std::cout<<kv<<","<<z<<",,"<<kv*pas_theta_v<<std::endl;
        for(int ku=0;ku<size_u_param+3;++ku){
            //std::cout<<ku<<",,"<<ku*pas_theta_u<<","<<kv*pas_theta_v<<std::endl;
            float x = r_param*cos(ku*pas_theta_u-M_PI)*cos(kv*pas_theta_v-M_PI/2-pas_theta_v);
            float y = r_param*sin(ku*pas_theta_u-M_PI)*cos(kv*pas_theta_v-M_PI/2-pas_theta_v);
...
void polygon_grid::set_catenoide(int const size_u_param,int const size_v_param, float r_param, float h_param){
    resize(size_u_param+1,size_v_param+3);

    
    float pas_theta_u = h_param/size_u_param;
    float pas_theta_v = 2*M_PI/(size_v_param);

    auto& S=*this;

    for(int ku=0;ku<size_u_param+1;++ku){
            float z = r_param*(ku*pas_theta_u-h_param/2);
        //std::cout<<kv<<","<<z<<",,"<<kv*pas_theta_v<<std::endl;
            for(int kv=0;kv<size_v_param+3;++kv){
            std::cout<<ku<<",,"<<ku*pas_theta_u<<","<<std::endl;

            float x = r_param*cosh(ku*pas_theta_u-h_param/2)*cos(kv*pas_theta_v);
            float y = r_param*cosh(ku*pas_theta_u-h_param/2)*sin(kv*pas_theta_v);
...
void polygon_grid::set_helicoide(int const size_u_param,int const size_v_param, float n_spire_param, float l_param, float h_spire_param){
    resize(size_u_param,size_v_param);

    
    float pas_theta_u = l_param/size_u_param;
    float pas_theta_v = n_spire_param*2*M_PI/(size_v_param);

    auto& S=*this;

    for(int kv=0;kv<size_v_param;++kv){
        float z = h_spire_param*(kv*pas_theta_v);
        //std::cout<<kv<<","<<z<<",,"<<kv*pas_theta_v<<std::endl;
            for(int ku=0;ku<size_v_param;++ku){
            std::cout<<ku<<",,"<<ku*pas_theta_u<<","<<std::endl;

            float x = ku*pas_theta_u*cos(kv*pas_theta_v);
            float y = ku*pas_theta_u*sin(kv*pas_theta_v);
...
void polygon_grid::set_pseudo_sphere(int const size_u_param,int const size_v_param, float r_param, float h_param){
    resize(size_u_param+1,size_v_param+3);
    ...
    for(int ku=0;ku<size_u_param+1;++ku){
            float z = r_param*((ku*pas_theta_u-h_param/2)-tanh(ku*pas_theta_u-h_param/2));
        //std::cout<<kv<<","<<z<<",,"<<kv*pas_theta_v<<std::endl;
            for(int kv=0;kv<size_v_param+3;++kv){
            std::cout<<ku<<",,"<<ku*pas_theta_u<<","<<std::endl;

            float x = r_param*cos(kv*pas_theta_v)/cosh(ku*pas_theta_u-h_param/2);
            float y = r_param*sin(kv*pas_theta_v)/cosh(ku*pas_theta_u-h_param/2);
...
void polygon_grid::set_hyperbolique(int const size_u_param,int const size_v_param, float a_param, float b_param, float h_param){
    resize(size_u_param,size_v_param);

    float pas_theta_u = a_param/size_u_param;
    float pas_theta_v = b_param/(size_v_param);
    ...
    for(int kv=0;kv<size_v_param;++kv){
        for(int ku=0;ku<size_u_param;++ku){
            //std::cout<<ku<<",,"<<ku*pas_theta_u<<","<<kv*pas_theta_v<<std::endl;
            float x = a_param*ku*pas_theta_u;
            float y = b_param*kv*pas_theta_v;
            float z = h_param*(ku*pas_theta_u*ku*pas_theta_u-kv*pas_theta_v*kv*pas_theta_v);
            
            //std::cout<<x<<",,"<<y<<","<<zb_param<<std::endl;
            S(ku,kv)={x,y,z};
        }
    }
...
void polygon_grid::set_visage(int const size_u_param,int const size_v_param, float r_param, float h_nez){
    resize(size_u_param+3,size_v_param+3);

    float pas_theta_u = 2*M_PI/size_u_param;
    float pas_theta_v = M_PI/(size_v_param);
    int yeux_z = (2.0/5.0)*size_v_param;
    int yeux_x1 = size_u_param/2.0-(1.0/15.0)*size_u_param;
    int yeux_x2 = size_u_param/2.0+(1.0/15.0)*size_u_param;
    int nez_z = size_v_param/2.0;
    int nez_x = size_u_param/2.0;
    int bouche_z = (3.0/5.0)*size_v_param+1;
    int bouche_x1 = size_u_param/2.0-(1.0/15.0)*size_u_param;
    int bouche_x2 = size_u_param/2.0+(1.0/15.0)*size_u_param;
    std::cout<<"yeux"<<yeux_z<<std::endl;
    auto& S=*this;

    for(int kv=0;kv<size_v_param+3;++kv){
        float z = r_param*sin(kv*pas_theta_v-M_PI/2);
        //std::cout<<kv<<","<<z<<",,"<<kv*pas_theta_v<<std::endl;
        for(int ku=0;ku<size_u_param+3;++ku){
            float x = r_param*cos(ku*pas_theta_u-M_PI)*cos(kv*pas_theta_v-M_PI/2);
            float y = r_param*sin(ku*pas_theta_u-M_PI)*cos(kv*pas_theta_v-M_PI/2);
            if (kv==yeux_z){
                //std::cout<<"bloute"<<ku<<std::endl;
                if (ku==yeux_x1 || ku==yeux_x2){
                    std::cout<<"bloute"<<ku<<std::endl;
                    x=0.7*x;
                    y=0.7*y;
                //}
            ...
            if (kv==nez_z){
                if (ku==nez_x){
                    std::cout<<"2bloute"<<ku<<std::endl;
                    x=1.0*x;
                    y=1.0*y;
                }
            }
            if (kv==bouche_z){
                if ((ku>bouche_x1) && (ku<bouche_x2)){
                    std::cout<<"3bloute"<<ku<<std::endl;
                    x=0.7*x;
                    y=0.7*y;
                    if (kv==nez_z){
                        x=0.85*x;
            ...
            if (kv==bouche_z+1){
                if ((ku>bouche_x1+1) && (ku<bouche_x2-1)){
                    std::cout<<"5bloute"<<ku<<std::endl;
                    x=0.7*x;
                    y=0.7*y;
                }
            }
...
void polygon_grid::set_tore(int const size_u_param,int const size_v_param, float r_int_param, float r_ext_param){
    resize(size_u_param+3,size_v_param+3);

    float pas_theta_u = 2.0f*M_PI/size_u_param;
    float pas_theta_v = 2.0f*M_PI/size_v_param;

    auto& S=*this;

    for(int ku=0;ku<=size_u_param+2;++ku){
        for(int kv=0;kv<=size_v_param+2;++kv){

            float x = (r_ext_param+r_int_param*cos(kv*pas_theta_v-M_PI))*cos(ku*pas_theta_u-M_PI);
            float y = (r_ext_param+r_int_param*cos(kv*pas_theta_v-M_PI))*sin(ku*pas_theta_u-M_PI);
            float z = r_int_param*sin(kv*pas_theta_v-M_PI);

...
void scene::load_scene()
{
    ...
    //control_polygon.set_cylinder(5,4,2.0f,1.0f);
    //control_polygon.set_tore(9,9,0.025f,1.0f);

    control_polygon.set_boule(15,15,0.5f);

    //control_polygon.set_visage(31,31,0.5f,0.7f);
    //control_polygon.set_catenoide(15,11,0.1f, 3.0f);
    //control_polygon.set_helicoide(15,15, 3.0, 0.5, 0.1);
    //control_polygon.set_pseudo_sphere(15,15,1.0,7.0);
    //control_polygon.set_hyperbolique(11,10,1.0f,1.5f,0.5f) ;
    ...
    //spline.surface(8,8).transform_apply_rotation({1.0f,0.0f,0.0f},-M_PI/2.0f);PRINT_OPENGL_ERROR();//
...
void scene::anime_visage()
{
    compteur=compteur+1;

    if (compteur%6==0){
        load_control_polygonlong1();
        up_to_date=false;
    }
    if (compteur%6==1){
        load_control_polygonlong2();
        up_to_date=false;
    }
    if (compteur%6==2){
        load_control_polygonlong3();
        up_to_date=false;
    }
    if (compteur%6==3){
        load_control_polygonlong4();
        up_to_date=false;
    }
    if (compteur%6==4){
        load_control_polygonlong3();
...
void scene::draw_scene()
{
    ...
    //auto const& p=control_polygon(0,0);
    //glUniform3f(get_uni_loc(shader_sphere,"translation"),p.x(),p.y(),p.z());
...
void scene::save_control_polygon() {control_polygon.save_file("control_polygonlong1.txt");}
void scene::load_control_polygon() {control_polygon.load_file("control_polygon.txt");recompute_surface();}
void scene::load_control_polygonlong1() {control_polygon.load_file("control_polygonlong1.txt");recompute_surface();}
void scene::load_control_polygonlong2() {control_polygon.load_file("control_polygonlong2.txt");recompute_surface();}
void scene::load_control_polygonlong3() {control_polygon.load_file("control_polygonlong3.txt");recompute_surface();}
void scene::load_control_polygonlong4() {control_polygon.load_file("control_polygonlong4.txt");recompute_surface();}
